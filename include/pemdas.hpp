...
double parse(std::string data, int debug=1);

std::map<char,char> lookAlikes = {}; // symbols that might be the same as other ones

std::vector<Token> tokenize(std::string data, int debug){ // tokenize the string
    ...
    for (int i=0; i < data.size(); i++){
        ...
        if (data.at(i) == '('){ 
            ...
            }  data.replace(data.begin()+i,data.begin()+j+1, std::to_string( parse(  before +  col , 0) ) ); // parse whatever is in the parenthesis and replace the parenthesis with the answer
        }
    }
...
double parse(std::string data, int debug){
    double output=0; std::vector<Token> tokens = tokenize(data,debug); // get the tokens
    bool init=false;
        ...
        for (int i=0; i < tokens.size(); i++){ // go through the tokens
            if (isIn(charbuffer+tokens.at(i).data,pemdas.at(p))){ // if the symbol is the same one as the current pemdas symbol
                ...
                switch (tokens.at(i).data.at(0)){
                    case '^': output = pow(output, std::stod(tokens.at(i+1).data)); if (!init && tokens.at(i-1).t == NUM) output= std::stod(tokens.at(i-1).data); break; // handle exponents
                    case 'q': output = sqrt(output); break; // handle square roots
                    case '*': output *= std::stod(tokens.at(i+1).data); if (!init && tokens.at(i-1).t == NUM) output= std::stod(tokens.at(i-1).data); break; // handle multiplication
                    case '/': output /= std::stod(tokens.at(i+1).data); if (!init && tokens.at(i-1).t == NUM) output= std::stod(tokens.at(i-1).data); break; // handle division 
                    case '+': output += std::stod(tokens.at(i+1).data); if (!init && tokens.at(i-1).t == NUM) output= std::stod(tokens.at(i-1).data); break; // handle addition
                    case '-': output -= std::stod(tokens.at(i+1).data); if (!init && tokens.at(i-1).t == NUM) output= std::stod(tokens.at(i-1).data); break; // handle subtraction
                    default: break;
                } 
            }
        }
